...
class _SinglePlayerScreenActivityState
  ...
  void _onTimerExpired() {
    if (_isGameOver || _isDisposed || !mounted) return;
    if (gameStatus != "started") return;
    
    // Only penalize human when it's their turn
    if (currentMove == utils.getTranslated(context, "yourTurn") && !_isGameOver) {
      _isGameOver = true;
      gameStatus = "over";
      _stopTimer();
      music.play(losegame);
      ...
      setState(() {});
    } else if (currentMove == utils.getTranslated(context, "doraTurn") && !_isGameOver) {
      // AI's turn but timer expired - AI might be stuck, restart timer
  ...
  void check() {
    // Guard: once the game is over, never re-enter
    if (gameStatus != "started" || _isGameOver) return;
  ...
  void checkTie(int totalBoxes) {
    if (_isGameOver || gameStatus != "started") return;
    ...
    if (_count == totalBoxes && winner == "0") {
      ...
      Future.delayed(const Duration(seconds: 1)).then((_) async {
        ...
        if (winner != "0" || gameStatus != "tie") return;
  ...
  void _handleGameOver(String winner, int totalBoxes) {
  ...
  void playGame([int? i]) async {
    ...
    if (_isProcessingMove || _isGameOver) return;
    if (gameStatus != "started") return;
    ...
    try {
      if (player == "X") {
        ...
        await Future.delayed(Duration(milliseconds: 500 + Random().nextInt(1000))).then((_) async {
          if (!mounted || _isDisposed || _isGameOver || gameStatus != "started") {
          ...
          final List currentBoardState = List.generate(totalCells, (index) {
            if (buttons[index]["state"] == "") return index;
            return buttons[index]["player"] == "2" ? "X" : "O";
          });

          final int aiMove = doraAI.getBestMove(
              currentBoardState, boardSize, widget.levelType ?? 0);

          if (!_isGameOver && aiMove >= 0 && aiMove < totalCells && buttons[aiMove]["state"] == "") {
            music.play(dice);
            buttons[aiMove]["state"] = "true";
            buttons[aiMove]["player"] = "2";
            
            // Check if AI won
            check();
            
            if (!_isGameOver && gameStatus == "started") {
              player = "O";
  ...
  void initState() {
    ...
    if (player == "O") {
      ...
      Future.delayed(const Duration(milliseconds: 500), () {
        if (!_isGameOver && mounted) playGame();
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop && !_isDisposed) showQuitGameDialog();
      },
      child: Scaffold(
