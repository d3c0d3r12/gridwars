...
class _ArcadeLobbyScreenState extends State<ArcadeLobbyScreen> {
  ...
  Future<void> _find() async {
    ...
    _timeout = Timer(const Duration(seconds: 60), () {
      if (_disposed || !mounted || _gameCancelled) return;
      _cancelAllSubscriptions();
      ArcadeService.cancelGame(widget.gameType, gameId);
      setState(() { 
        _searching = false; 
        _foundOpp = false;
        _status = 'No opponent found.\nTap to try again.'; 
      });
    });
    ...
    _sub = FirebaseDatabase.instance
        ...
        .onValue
        .listen((ev) async {
      if (_disposed || _navigated || _gameCancelled || ev.snapshot.value != 'active' || !mounted) return;
      
      _navigated = true;
      _cancelAllSubscriptions();

      // Deduct coins with proper error handling
      try {
        await FirebaseDatabase.instance.ref()
            .child('users').child(_uid).child('coin')
            .runTransaction((v) => Transaction.success((v as int? ?? 0) - fixedEntryFee));
      } catch (e) {
        // If coin deduction fails, still allow game? Better to cancel.
        _navigated = false;
        _gameCancelled = true;
        if (mounted) {
          setState(() {
            _searching = false;
            _status = 'Insufficient coins!\nTap to try again.';
          });
        }
        return;
      }
      ...
      final info = await ArcadeService.userInfo(oppId);
      if (mounted && !_disposed && !_gameCancelled) {
        _flashAndNavigate(gameId, isP1: true, oppId: oppId, oppName: info['username'] ?? 'Opponent');
      }
    });
    ...
    _lobbySub = FirebaseDatabase.instance
        ...
        .onChildAdded
        .listen((ev) async {
      if (_disposed || _navigated || _gameCancelled || !mounted) return;
      ...
      if (!ok || _disposed || !mounted || _navigated || _gameCancelled) return;
      ...
      if (mounted && !_disposed && !_gameCancelled) {
        _flashAndNavigate(otherKey, isP1: false, oppId: hostUid, oppName: info['username'] ?? 'Opponent');
      }
    });
  }
  ...
  void _flashAndNavigate(String gameId, {required bool isP1, required String oppId, required String oppName}) {
    ...
    _navDelay = Timer(const Duration(milliseconds: 1200), () {
      if (!_disposed && mounted && !_gameCancelled) {
        _navigate(gameId, isP1: isP1, oppId: oppId, oppName: oppName);
      }
    });
  }

  void _navigate(String gameId, {required bool isP1, required String oppId, required String oppName}) {
    if (_disposed || _gameCancelled) return;
    
    Widget screen;
    final args = _GameArgs(
      gameId: gameId, 
  ...
  void _handleBackPress() async {
    _cancelAllSubscriptions();
    if (_gameId.isNotEmpty && _searching && !_gameCancelled) {
      _gameCancelled = true;
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop && !_disposed) _handleBackPress();
      },
      child: Scaffold(
            ...
            Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              AnimatedSwitcher(
                ...
                child: Container(
                  ...
                  child: Center(child: _searching && !_foundOpp
              ...
              if (!_searching && !_foundOpp && !_gameCancelled)
                GestureDetector(
                  onTap: _retry,
                  child: Container(
