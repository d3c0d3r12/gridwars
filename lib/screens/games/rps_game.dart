...
import 'game_widgets.dart' show showLeaveConfirmDialog, showOpponentLeftDialog;
...
class _RpsGameScreenState extends State<RpsGameScreen> {
  ...
  void initState() {
    ...
    _sub = ArcadeService.stateRef(widget.args.type, widget.args.gameId)
      ...
      if (data['status'] == 'finished' || data['status'] == 'cancelled') {
        _sub?.cancel();
        // If opponent abandoned (not a normal game-over we triggered), show dialog.
        if (!_gameOver && !_abandoned && mounted) {
  ...
  void _abandonGame() async {
    if (_gameOver || _abandoned) return;
    _abandoned = true;
    setState(() => _gameOver = true);
    _sub?.cancel();
  ...
  void _checkRound(Map st) {
    ...
    Future.delayed(const Duration(milliseconds: 400), () async {
      ...
      setState(() { _resultMsg = roundResult; _myChoice = ''; });

      if (round >= maxR || p1Score > maxR ~/ 2 || p2Score > maxR ~/ 2) {
        // Game over
        String? winner;
        ...
        if (mounted) _showResult(p1Score, p2Score);
      } else {
        if (widget.args.isP1) {
          await ArcadeService.updateState(widget.args.type, widget.args.gameId,
              {'p1Score': p1Score, 'p2Score': p2Score, 'round': round + 1, 'p1Choice': '', 'p2Choice': ''});
  ...
  void _showResult(int p1Score, int p2Score) {
    ...
    showDialog(context: context, barrierDismissible: false, builder: (_) => AlertDialog(
      ...
      title: Text(won ? '🏆 You Win!' : myScore == oppScore ? '🤝 Draw!' : '😔 You Lose', style: TextStyle(color: white, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
    ...
    return PopScope(
      ...
      body: Container(
        ...
        child: SafeArea(child: Column(children: [
          ...
          if (_myChoice.isNotEmpty && !oppPicked)
...
// ── Shared header/pill helpers ─────────────────────────────────────────────

Widget _header(BuildContext ctx, String title, String sub, int myScore, int oppScore, {VoidCallback? onExit}) {
  return Padding(
    padding: const EdgeInsets.fromLTRB(16, 10, 16, 0),
    child: Row(children: [
      GestureDetector(onTap: onExit ?? () => Navigator.pop(ctx), child: Icon(Icons.close, color: white.withValues(alpha: 0.7))),
      const Spacer(),
      Column(children: [
